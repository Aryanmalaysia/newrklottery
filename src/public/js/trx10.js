function showListOrder3(list_orders, x) {
    ...
    let result = list_orders.map((list_orders) => {
      return (htmls += `
         ...
         <span>***578</span>
          ...
          <span data-v-a9660e98=""> ${list_orders.amount < 5 ? "S" : "B"
          } </span>
                            </div>
                        </div>
    ...
    let htmlchart=""
    let chart = list_orders.map((list_orders) => {
     return (htmlchart += `
               
                 <li>
                     <div class="first">${list_orders.period
       }</div>
                     <div class="sec">
                        <span class="${list_orders.amount ==0 ? `active zero `:"" }">0</span>
                         <span class="${list_orders.amount ==1 ?`active one`:"" }">1</span>
                         <span class="${list_orders.amount ==2 ? `active ${list_orders.amount}`:"" }">2</span>
                         <span class="${list_orders.amount ==3 ?`active ${list_orders.amount}`:"" }">3</span>
                         <span class="${list_orders.amount ==4 ? `active ${list_orders.amount}`:"" }">4</span>
                         <span class="${list_orders.amount ==5 ?`active ${list_orders.amount}`:"" }">5</span>
                         <span class="${list_orders.amount ==6 ? `active ${list_orders.amount}`:"" }">6</span>
                         <span class="${list_orders.amount ==7 ? `active ${list_orders.amount}`:"" }">7</span>
                         <span class="${list_orders.amount ==8 ?`active ${list_orders.amount}`:"" }">8</span>
                         <span class="${list_orders.amount ==9 ? `active ${list_orders.amount}`:"" }">9</span>
                     </div>
                     <div class="third">${list_orders.amount < 5 ? "S" : "B"
       }</div>
                 </li>
  ...
  $("#trendList").prepend(htmlchart);
   $("#trendList li").last().remove();
...
var myModal_result_Period = document.getElementById("myModal_result_Period");
        if (firstGame.get == 0) {
            ...
            $("#popupImage").attr("src", "/loss-popup.png");
        } else {
    ...
    showListOrder_t(list_orders, 2);
      ...
      .then((data) => {
        ...
        $(".num span").text(`₹ ${Number(data.data.money_user).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`);
      });
      $(".Loading").fadeOut(0);
    }, 1000);
  });
  ...
  $(".foot .right").click(function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $(".total-box .num span").text("₹ " + Number(response.money).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}));
  ...
  function showListOrder(list_orders, x) {
    ...
    const container = document.getElementById('wingo-change-time-img');
    ...
    list_orders.forEach(order => {
      ...
      if (count < 5 && !uniqueAmounts[amount]) {
        const img = document.createElement('img');
        img.src = imageMapping[amount];
        img.alt = `Image ${amount}`;
        container.appendChild(img);
  
        uniqueAmounts[amount] = false;
        count++;
      }
    });
    ...
    let htmls = "";
    let result = list_orders.map((list_orders) => {
      const date = new Date(Number(list_orders.time)); // Convert microseconds to milliseconds
  
      const hours = date.getHours();
      const minutes = date.getMinutes();
      const seconds = date.getSeconds();
  
      const hashs = list_orders.time.slice(10)
         ...
         <span>***${hashs}</span>
                    <!---->
                        ...
                        <div data-v-a9660e98="" class="van-col van-col--5">
                            <div data-v-a9660e98="" class="c-tc goItem">
                                <!---->
                                <span data-v-a9660e98="" class="${list_orders.amount % 2 == 0 ? list_orders.amount === 0 ? "red-violet" : "red" : list_orders.amount === 5 ? "green-violet" : "green"
          }"> ${list_orders.amount} </span>
  
          <span data-v-a9660e98="" class="${list_orders.amount < 5 ? "blue" : "orange"}"> ${list_orders.amount < 5 ? "S" : "B"
          } </span>
                            </div>
                        </div>
                    </div>
   ...
   return (htmlchart += `
               ...
               <li>
                   ...
                   <div class="sec">
                       <span class="${list_orders.amount ==0 ?`active zero`:"" }">0</span>
                       <span class="${list_orders.amount ==1 ?`active one`:"" }">1</span>
                       <span class="${list_orders.amount ==2 ? `active two`:"" }">2</span>
                       <span class="${list_orders.amount ==3 ?`active three`:"" }">3</span>
                       <span class="${list_orders.amount ==4 ? `active four`:"" }">4</span>
                       <span class="${list_orders.amount ==5 ?`active five`:"" }">5</span>
                       <span class="${list_orders.amount ==6 ? `active six`:"" }">6</span>
                       <span class="${list_orders.amount ==7 ? `active seven`:"" }">7</span>
                       <span class="${list_orders.amount ==8 ?`active eight`:"" }">8</span>
                       <span class="${list_orders.amount ==9 ? `active nine`:"" }">9</span>
                   </div>
                   <div class="third ${list_orders.amount < 5 ? "color-blue" : "color-yellow"}" >${list_orders.amount < 5 ? "S" : "B"
     }</div>
               </li>
                          ...
                          <div data-v-a9660e98="" class="item c-row">
                              ...
                              <div data-v-a9660e98="" class="c-row c-row-between info">
                                ...
                                ${list_orders.status == 1
                                  ? '<span data-v-a9660e98="" class="state green">Succeed</span>'
                                  : list_orders.status == 2
                                  ? '<span data-v-a9660e98="" class="state red">Failed</span>'
                                  : ""
                                  }

                                ${list_orders.status == 1
                                  ? '<span data-v-a9660e98="" class="success"> + ' + "₹" +
                                  Number(list_orders.get).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) +
                                  " </span>"
                                  : list_orders.status == 2
                                  ? '<span data-v-a9660e98="" class="fail"> - ' + "₹" +
                                  Number(list_orders.money).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) +
                                  "</span>"
                                  : ""
                                  }
