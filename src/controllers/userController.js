...
const userInfo = async (req, res) => {
    ...
    return res.status(200).json({
        ...
        data: {
            ...
            money_user_usdt: parseInt(parseFloat(others.money) / 93) || 0,
        },
...
const promotion = async (req, res) => {
    ...
    for (let i = 0; i < f1s.length; i++) {
        ...
        phoneData.push(f1s[i].phone)
        let check = (timerJoin(f1_time) == timerJoin()) ? true : false;
        if (check) {
            f1_today += 1;
        }
    }
    const validPhoneNumbers = phoneData.filter(phone => phone && /^\d{10}$/.test(phone));
    ...
    let totalRecharge = 0;
    let recharge = []
    if (validPhoneNumbers && validPhoneNumbers.length) {
        [recharge] = await connection.query(
            'SELECT * FROM recharge WHERE `phone` IN (?) AND `status` = 1 AND time >= ? AND time < ?',
            [validPhoneNumbers, today.getTime(), tomorrow.getTime()]
        );
        totalDeposite = recharge.length;
        recharge.forEach(data => {
            totalRecharge += data.money;
        });
    }
    ...
    let totalLevelRecharge = 0;

    let recharge2 = []
    let totalLevelDeposite = 0
    let phoneNumbers = selectedData.map(x => x.phone).filter(phone => phone && /^\d{10}$/.test(phone));
    if (phoneNumbers && phoneNumbers.length) {
        let [recharge2] = await connection.query(
            'SELECT * FROM recharge WHERE `phone` IN (?) AND `status` = 1 AND time >= ? AND time < ?',
            [phoneNumbers, today.getTime(), tomorrow.getTime()]
        );
        totalLevelDeposite = recharge2.length;
        recharge2.forEach(data => {
            totalLevelRecharge += data.money;
        });
    }
    return res.status(200).json({
...
const addBank = async (req, res) => {
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        return res.status(200).json({
            message: 'Successfully upadted bank',
            status: false,
            timeStamp: timeNow,
        });
...
const addCryptoAddress = async (req, res) => {
    let auth = req.cookies.auth; // Assuming authentication token is stored in cookies
    let erc20Address = req.body.erc20Address; // ERC-20 Address
    let bnbAddress = req.body.bnbAddress; // BNB Address
    let maticAddress = req.body.maticAddress; // Matic Address
    let trc20Address = req.body.trc20Address; // TRC-20 Address

    let timeNow = new Date().getTime();

    if (!auth || (!erc20Address && !bnbAddress && !maticAddress && !trc20Address)) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
    ...
    if (user_bank2.length) {
        await connection.query('UPDATE user_bank SET erc20_address = ?, bnb_address = ?, matic_address = ?, trc20_address = ?, time = ? WHERE phone = ? ',
            [erc20Address, bnbAddress, maticAddress, trc20Address, timeNow, userInfo.phone]);
        ...
        (phone, erc20_address, bnb_address, matic_address, trc20_address, time)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        await connection.execute(sql, [userInfo.phone, erc20Address || null, bnbAddress || null, maticAddress || null, trc20Address || null, timeNow]);
...
const withdrawalDetails = async (req, res) => {
    try {
        let auth = req.cookies.auth;   
        const [user] = await connection.query('SELECT `phone`, `code`, `invite` FROM users WHERE `token` = ? ', [auth]);
        let userInfo = user[0];
        ...
        const [bankDetails] = await connection.query('SELECT * FROM user_bank WHERE `phone` = ? ', [userInfo.phone]);
        ...
        const { name_bank, name_user, stk, email, sdt, tinh, erc20_address, bnb_address, matic_address, trc20_address } = bankDetails[0];
...
const withdrawal3 = async (req, res) => {
    ...
    let money = 0
    let password = req.body.password;
    let paymentsType = req.body.paymentsType

    if (paymentsType == 'usdt') {
        money = req.body.money / 95
    } else {
        money = req.body.money
    }
    if (!auth || !money || !password || !paymentsType) {
        return res.status(200).json({
    ...
    const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ? AND time >= ? AND time < ?', [userInfo.phone, today.getTime(), tomorrow.getTime()]);
    if (user_bank.length != 0) {
                ...
                if (result == 0) {
                    if (total - total2 >= 0) {
                        ...
                        INSERT INTO withdraw 
                            (id_order, phone, type, money, stk, sdt, name_bank, ifsc, name_user, status, today, time, erc20_address, bnb_address, trc20_address, matic_address)
                        VALUES 
                            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    `;
                    
                    await connection.execute(sql, [
                        id_order,                // Value for id_order column
                        userInfo.phone,          // Value for phone column
                        paymentsType,            // Value for type column
                        money,                   // Value for money column
                        infoBank.stk,            // Value for stk column
                        infoBank.sdt,            // Value for sdt column
                        infoBank.name_bank,      // Value for name_bank column
                        infoBank.email,           // Value for ifsc column
                        infoBank.name_user,      // Value for name_user column
                        0,                       // Value for status column (assuming default value)
                        checkTime,               // Value for today column (assuming it's a date)
                        dates,                   // Value for time column (assuming it's a date or time)
                        infoBank.erc20_address,  // Value for erc20_address column
                        infoBank.bnb_address,    // Value for bnb_address column
                        infoBank.trc20_address,  // Value for trc20_address column
                        infoBank.matic_address   // Value for matic_address column
                    ]);
        ...
        return res.status(200).json({
            message: 'Please link your bank or crypto adddress details',
            status: false,
            timeStamp: timeNow,
        });
    }
...
const downlinerecharge = async (req, res) => {
  ...
  let better_number = 0;
  async function fetchUserrechagreByCode() {
      ...
      try {
        const [userCombinedTotal] = await connection.query(
          `
             ...
             `,
          [selectedData[i].phone, selectedData[i].phone, selectedData[i].phone]
        );

        const [rechargeRecord] = await connection.query(
          `
                 SELECT IFNULL(SUM(\`money\`), 0) as grand_total_money FROM \`recharge\` WHERE \`phone\` = ? AND \`status\` = 1
             `,
          [selectedData[i].phone]
        );

        const [deposits] = await connection.query(
          "SELECT `id`, `id_order`, `transaction_id`, `utr`, `phone`, `money`, `type`, `status`, `today`, `url`, `time` FROM `recharge` WHERE `phone` = ? AND `status` = 1",
          [selectedData[i].phone]
        );
        const [userCombinedTotal_count] = await connection.query(
          `
                 ...
                 FROM (
                     SELECT phone FROM minutes_1 WHERE \`phone\` = ? 
                     UNION ALL
                     SELECT phone FROM result_k3 WHERE \`phone\` = ? 
                     UNION ALL
                     SELECT phone FROM result_5d WHERE \`phone\` = ? 
                 ) AS combined_table
             `,
          [selectedData[i].phone, selectedData[i].phone, selectedData[i].phone]
        );
        ...
        total_bet_amount += parseFloat(userCombinedTotal[0].grand_total_money);

        const userObject = {
          totalBet: grandTotalMoney,
          grandRechargeTotalMoney: grandRechargeTotalMoney,
          level: selectedData[i].level,
          userid: selectedData[i].id_user,
          time: selectedData[i].time,
        };
