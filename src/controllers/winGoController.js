            ...
            for (let levelIndex = 1; levelIndex <= 6; levelIndex++) {
                ...
                if (infoF1.user_level >= levelIndex && infoF1.total_money >= 100) {
                    ...
                    if (rosesF > 0) {
                        ...
                        const datasql = 'INSERT INTO transaction_history SET phone = ?, detail = ?, balance = ?, `time` = NOW()';
                        await connection.execute(datasql, [infoF1.phone, "Agent Commission", rosesF]);
...
const betWinGo = async (req, res) => {
    ...
    if (typeid != 1 && typeid != 3 && typeid != 5 && typeid != 10 && typeid != 11  && typeid != 33  && typeid != 55  && typeid != 100) {
    ...
    if (typeid == 11) gameJoin = 'trx';
    if (typeid == 33) gameJoin = 'trx3';
    if (typeid == 55) gameJoin = 'trx5';
    if (typeid == 100) gameJoin = 'trx10';
    ...
    if (check >= 0) {
        ...
        const datasql = 'INSERT INTO transaction_history SET phone = ?, detail = ?, balance = ?, `time` = NOW()';
        await connection.query(datasql, [userInfo.phone, "Bet", total]);
        // const [level] = await connection.query('SELECT * FROM level ');
        // let level0 = level[0];
        // const sql2 = `INSERT INTO roses SET 
        // phone = ?,
...
const listOrderOld = async (req, res) => {
    ...
    if (!pageno) {
        return res.status(200).json({
            message: 'Error! pageno is missing.',
            status: true
        });
    } else if (!pageto) {
        return res.status(200).json({
            message: 'Error! pageto is missing.',
            status: true
        });
    } else if (!user[0]) {
        return res.status(200).json({
        ...
        return res.status(200).json({
            message: 'Error! wingo is missing.',
            status: true
        });
    } else if (!period[0]) {
        return res.status(200).json({
...
const GetMyEmerdList = async (req, res) => {
    ...
    if (typeid != 1 && typeid != 3 && typeid != 5 && typeid != 10  && typeid != 11 && typeid != 33 && typeid != 55 && typeid != 100 && typeid != 9) {
    ...
    if (typeid == 9) {
        ...
        const [minutes_1] = await connection.query(`SELECT * FROM minutes_1 WHERE phone = ? ORDER BY id DESC`, [phone]);
    ...
    ORDER BY id DESC`, [phone]);
        ...
        const combinedData = [...minutes_1, ...result_5d, ...result_k3];
